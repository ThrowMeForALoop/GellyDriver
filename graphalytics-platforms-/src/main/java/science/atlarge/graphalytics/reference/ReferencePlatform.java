...
import science.atlarge.graphalytics.reference.algorithms.sssp.SingleSourceShortestPathsJob;
import science.atlarge.graphalytics.reference.algorithms.wcc.WeaklyConnectedComponentsJob;
import java.nio.file.Paths;
import java.nio.file.Path;

/**
 * Reference platform driver for the Graphalytics benchmark.
 *
 * @author Unspecified
 */
public class ReferencePlatform implements Platform {
	...
	public static final String PLATFORM_NAME = "reference";
	...
	public LoadedGraph loadGraph(FormattedGraph formattedGraph) throws Exception {
		ReferenceConfiguration platformConfig = ReferenceConfiguration.parsePropertiesFile();
		loader = new ReferenceLoader(formattedGraph, platformConfig);
		...
		try {
			...
			if (exitCode != 0) {
				throw new PlatformExecutionException("Reference exited with an error code: " + exitCode);
			}
		} catch (Exception e) {
			throw new PlatformExecutionException("Failed to load a Reference dataset.", e);
		}
	...
	public void deleteGraph(LoadedGraph loadedGraph) throws Exception {
		...
		try {
			...
			throw new PlatformExecutionException("Failed to unload a Reference dataset.", e);
		}
	...
	public void startup(RunSpecification runSpecification) throws Exception {
		...
		ReferenceCollector.startPlatformLogging(logDir);
	}
	...
	public void run(RunSpecification runSpecification) throws PlatformExecutionException {
		...
		try {
			...
			throw new PlatformExecutionException("Failed to execute a Reference job.", e);
		}
	...
	public BenchmarkMetrics finalize(RunSpecification runSpecification) throws Exception {
		ReferenceCollector.stopPlatformLogging();
		BenchmarkRunSetup benchmarkRunSetup = runSpecification.getBenchmarkRunSetup();
		Path logDir = benchmarkRunSetup.getLogDir().resolve("platform");

		BenchmarkMetrics metrics = new BenchmarkMetrics();
		metrics.setProcessingTime(ReferenceCollector.collectProcessingTime(logDir));
		return metrics;
	}
